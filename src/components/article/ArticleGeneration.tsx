import React, { useState, useEffect, useCallback } from 'react';
import { useOpenAI } from '../../context/OpenAIContext';
import ArticlePreview from './ArticlePreview';
import GenerationProgress from './GenerationProgress';
import { Loader2, ChevronDown } from 'lucide-react';
import { toast } from 'sonner';
import { generateArticle } from '../../utils/articleGenerator';

export default function ArticleGeneration() {
  const { state, generateArticle: updateArticleState } = useOpenAI();
  const [isGenerating, setIsGenerating] = useState(false);
  const [currentPhase, setCurrentPhase] = useState<string>('');
  const [progress, setProgress] = useState(0);
  const [isExpanded, setIsExpanded] = useState(true);

  const handleExpand = useCallback(() => {
    if (state.articleGeneration.content || isGenerating) {
      setIsExpanded(!isExpanded);
    }
  }, [isExpanded, state.articleGeneration.content, isGenerating]);

  useEffect(() => {
    if (state.researchThreads.outline?.completed && !state.articleGeneration.content && !isGenerating) {
      handleGenerate();
    }
  }, [state.researchThreads.outline?.completed]);

  const handleGenerate = async () => {
    setIsGenerating(true);
    setProgress(0);
    setCurrentPhase('Preparing research data');

    try {
      await simulatePhase('Synthesizing research findings', 20);
      await simulatePhase('Generating content structure', 40);
      await simulatePhase('Writing main content', 60);
      await simulatePhase('Optimizing for SEO', 80);
      await updateArticleState('start_generation');
      setProgress(100);
      setCurrentPhase('Article completed');
    } catch (error) {
      toast.error('Failed to generate article', {
        description: error instanceof Error ? error.message : 'An unexpected error occurred'
      });
    } finally {
      setTimeout(() => {
        setIsGenerating(false);
      }, 1000);
    }
  };

  const simulatePhase = async (phase: string, targetProgress: number) => {
    setCurrentPhase(phase);
    const increment = (targetProgress - progress) / 10;
    for (let i = 0; i < 10; i++) {
      await new Promise(resolve => setTimeout(resolve, 200));
      setProgress(prev => Math.min(prev + increment, targetProgress));
    }
  };

  if (!state.researchThreads.outline?.completed) {
    return (
      <div className="text-center py-8">
        <p className="text-zinc-400 mb-4">
          Complete all research phases to generate your article
        </p>
        <p className="text-sm text-zinc-500">
          Start by entering a keyword above and completing the research process
        </p>
      </div>
    );
  }

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-xl font-semibold text-white">
            {state.userContent.articleTitle || 'Article Generation'}
          </h2>
          <p className="text-sm font-medium text-emerald-500 mt-1">
            Article Generation
          </p>
          <p className="text-sm text-zinc-400 mt-1">
            {isGenerating ? 'Generating article...' : 
             state.articleGeneration.content ? 'Article generated successfully' :
             'Ready to generate article'}
          </p>
        </div>
        
        <button
          onClick={handleExpand}
          className={`p-2 hover:bg-zinc-800 rounded-lg transition-colors ${
            !state.articleGeneration.content && !isGenerating ? 'opacity-50 cursor-not-allowed' : ''
          }`}
          disabled={!state.articleGeneration.content && !isGenerating}
        >
          <ChevronDown 
            className={`w-5 h-5 text-zinc-400 transition-transform duration-200
              ${isExpanded ? 'transform rotate-180' : ''}`}
          />
        </button>
      </div>

      <div className={`space-y-6 transition-all duration-300 ${
        isExpanded ? 'opacity-100' : 'opacity-0 h-0 overflow-hidden'
      }`}>
        {isGenerating && (
          <GenerationProgress
            phase={currentPhase}
            progress={progress}
          />
        )}

        {state.articleGeneration.content && (
          <ArticlePreview content={state.articleGeneration.content} />
        )}
      </div>
    </div>
  );
}